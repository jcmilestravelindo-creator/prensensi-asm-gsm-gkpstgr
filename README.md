...
<body>
    ...
    <script type="text/babel" data-type="module">
        ...
        const Scanner = ({ user }) => {
            ...
            const onScan = (code) => {
                if (isProcessing.current || status.type !== 'idle') return;
                
                // GUARD: Jangan proses jika kode kosong
                const searchCode = code ? String(code).toLowerCase().trim() : '';
                ...
                } catch(e) { console.log("Pause warning:", e); }

                let users = DB.get(STORAGE.USERS);
                if (!users || users.length === 0) users = DEFAULT_USERS;

                const u = users.find(x => 
                    (x.qrCode && x.qrCode.toLowerCase() === searchCode) || 
                    (x.id && String(x.id).toLowerCase() === searchCode) || 
                    (x.name && x.name.toLowerCase().includes(searchCode))
                );
                
                let nextStatus = { type: 'idle', msg: '' };

                if (!u) {
                    nextStatus = { type: 'err', msg: 'Data Tidak Ditemukan' };
                } else {
                    let isAllowed = false;
                    let denyReason = '';
                    ...
                    if (user.role === 'ADMIN') {
                        ...
                        if (u.role === 'STUDENT') {
                            const teacherClass = user.className ? user.className.trim() : '';
                            const studentClass = u.className ? u.className.trim() : '';
                            
                            if (teacherClass === studentClass) {
                                isAllowed = true;
                            } else {
                                denyReason = `Siswa Kelas Lain (${u.className})`;
                            }
                        } else if (u.role === 'TEACHER') {
                             denyReason = 'Guru scan Guru tidak diizinkan';
                        } else {
                             denyReason = 'Akses ditolak';
                        }
                    } else {
                         denyReason = 'Anda tidak memiliki akses scan';
                    }
                    ...
                    if (isAllowed) {
                        ...
                        if (ok) {
                            ...
                            nextStatus = { type: 'ok', msg: `Berhasil: ${u.name}` };
                        } else {
                            nextStatus = { type: 'err', msg: `Sudah Absen Hari Ini` };
                        }
                    } else {
                        nextStatus = { type: 'err', msg: denyReason || 'Akses Ditolak' };
                    }
                }
                ...
                setTimeout(() => { 
                    ...
                    try { 
                        if(!manual && ref.current && ref.current.getState() === 3) {
                            ref.current.resume();
                        }
                    } catch(e) { console.log("Resume warning:", e); }
                }, 2000);
            };
            ...
            useEffect(() => {
                ...
                const init = async () => {
                    try {
                        ...
                        if(devs && devs.length) {
                            if(ref.current) {
                                try { await ref.current.stop(); await ref.current.clear(); } catch(e){}
                            }
                    ...
                    <div className="flex flex-col gap-6 w-full">
                                ...
                                {manual ? (
                                    <div className="w-full px-6 text-center relative z-10 animate-pop">
                                        ...
                                        <input 
                                            ...
                                            onKeyDown={(e) => { if (e.key === 'Enter') { onScan(txt); setTxt(''); } }}
                                        />
                                    ...
                                    <>
                                        ...
                                        <div className="absolute bottom-4 left-0 right-0 text-center">
                                            <span className="bg-black/40 backdrop-blur-md text-white/80 text-[10px] px-3 py-1 rounded-full border border-white/10">Arahkan kamera ke QR Code</span>
                                        </div>
                                    </>
                                )}
                        ...
                        <div className="z-20">
                            ...
                            >
                                ...
                                {manual ? 'Kembali ke Kamera' : 'Input Manual (Nama/ID)'}
                            </button>
                        </div>
                    </div>
                </div>
        ...
        const Dashboard = ({ user }) => {
            ...
            const weeklyData = [{ name: 'Min', hadir: att.length }]; 
            ...
            return (
                <div className="p-6 pb-32 animate-slide-up">
                            ...
                            <div onClick={() => setIsMatModalOpen(true)} className="glass-card p-5 rounded-3xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white cursor-pointer hover:shadow-xl hover:shadow-indigo-500/30 transition-all active:scale-95 group relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-indigo-100 text-xs font-medium opacity-90">Klik untuk buat</p>
                                    </div>
                                </div>
                            </div>
                            <div onClick={() => setIsSchedModalOpen(true)} className="glass-card p-5 rounded-3xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white cursor-pointer hover:shadow-xl hover:shadow-blue-500/30 transition-all active:scale-95 group relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-blue-100 text-xs font-medium opacity-90">Atur kegiatan</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        {att.map((r, i) => (
                            <div key={i} className="glass-card p-4 rounded-2xl flex items-center justify-between animate-pop">
                        ...
                        {att.length === 0 && <p className="text-center text-slate-400 py-8 text-sm col-span-full">Belum ada yang hadir.</p>}
                    </div>
            ...
            return (
                <div className="p-6 h-full flex flex-col pb-24 max-w-md mx-auto animate-slide-up">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button onClick={generateDevotional} className="bg-gradient-to-tr from-yellow-400 to-orange-500 text-white p-2 rounded-xl shadow-lg hover:scale-105 transition-transform" title="Renungan Harian AI">
                            {loadingDevo ? <RefreshCw size={20} className="animate-spin"/> : <Sparkles size={20}/>}
                        </button>
                    </div>
                    {aiDevotional && (
                        <div className="mb-6 bg-gradient-to-r from-yellow-50 to-orange-50 p-4 rounded-2xl border border-orange-100 shadow-sm animate-pop relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-16 h-16 bg-yellow-200 rounded-full blur-2xl opacity-50 -mr-4 -mt-4"></div>
                            <div className="relative z-10">
                                <h4 className="font-bold text-orange-600 text-sm flex items-center gap-2 mb-1"><Sparkles size={12}/> {aiDevotional.title}</h4>
                                <p className="text-xs text-slate-700 italic">"{aiDevotional.content}"</p>
                            </div>
                            <button onClick={()=>setAiDevotional(null)} className="absolute top-2 right-2 text-orange-300 hover:text-orange-500"><X size={14}/></button>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar">
                        ...
                        {tab === 'history' && (
                            <div className="space-y-4 animate-pop">
                                <h3 className="font-bold text-slate-700 mb-2 flex items-center gap-2"><Clock size={18} className="text-indigo-500"/> Riwayat Kehadiran</h3>
                                {history.length === 0 ? <div className="text-center py-10 text-slate-400 text-sm">Belum ada riwayat kehadiran.</div> : 
                                    history.map((h, i) => (
                                        <div key={h.id} className="glass-card p-4 rounded-2xl flex justify-between items-center border-l-4 border-l-emerald-500">
                                            ...
                                            <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full text-xs font-bold font-mono">{new Date(h.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                        </div>
                                    ))
                                }
                            </div>
                        ...
                        {tab === 'materials' && (
                            <div className="space-y-4 animate-pop">
                                <h3 className="font-bold text-slate-700 mb-2 flex items-center gap-2"><BookOpen size={18} className="text-indigo-500"/> Materi Pembelajaran</h3>
                                {materials.length === 0 ? <div className="text-center py-10 text-slate-400 text-sm">Belum ada materi dari guru.</div> : 
                                    materials.map((m, i) => (
                                        <div key={m.id} className="glass-card p-5 rounded-2xl relative overflow-hidden">
                                            <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-indigo-500 to-purple-500"></div>
                                            <div className="mb-3">
                                                <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider bg-indigo-50 px-2 py-1 rounded-md mb-2 inline-block">{new Date(m.createdAt).toLocaleDateString()}</span>
                        ...
                        {tab === 'schedule' && (
                            <div className="space-y-4 animate-pop">
                                <h3 className="font-bold text-slate-700 mb-2 flex items-center gap-2"><CalendarDays size={18} className="text-indigo-500"/> Jadwal {user.className}</h3>
                                {schedules.length === 0 ? <div className="text-center py-10 text-slate-400 text-sm">Belum ada jadwal untuk kelas ini.</div> : 
                                        ...
                                        {schedules.map((s, i) => (
                                            <div key={s.id} className="glass-card p-0 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-white/60">
                                                <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 border-b border-indigo-100 flex justify-between items-center">
                                                    ...
                                                    <span className="text-[10px] font-bold bg-white text-indigo-600 px-2 py-1 rounded-lg border border-indigo-100 shadow-sm">{s.status === 'upcoming' ? 'Akan Datang' : 'Selesai'}</span>
                                                </div>
                                                <div className="p-4 space-y-3">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-indigo-500"><BookOpen size={16} /></div>
                                                        <div><p className="text-[10px] text-slate-400 font-bold uppercase">Tema</p><p className="text-xs font-bold text-slate-700">{s.theme}</p></div>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div className="flex items-center gap-2"><Clock size={14} className="text-slate-400"/><span className="text-xs font-medium text-slate-600">{s.time}</span></div>
                                                        <div className="flex items-center gap-2"><MapPin size={14} className="text-slate-400"/><span className="text-xs font-medium text-slate-600">{s.location}</span></div>
                                                    </div>
                                                    <div className="flex items-center gap-2 pt-2 border-t border-slate-50"><User size={14} className="text-slate-400"/><span className="text-xs text-slate-500">Pengajar: <b>{s.speaker}</b></span></div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
            ...
            useEffect(() => {
                ...
                const unsubNotif = onSnapshot(getCollectionRef('notifications'), (snap) => {
                    ...
                    if (data.length > 0) {
                        ...
                        if(isNotifVisible(lastNotif)) setHasNewNotif(true);
                    }
                }, (e) => console.error(e));
                ...
                const initApp = async () => {
                    try {
                        ...
                        if (e.code === 'auth/configuration-not-found' || e.code === 'auth/admin-restricted-operation') {
                            alert("PERINGATAN: Mode Offline (Cek Config Firebase).");
                        }
                    } finally {
                        setIsReady(true);
                    }
