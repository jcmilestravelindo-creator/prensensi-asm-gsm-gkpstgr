...
// FIX: Normalisasi tanggal untuk pastikan perbandingan konsisten WIB
const todayWIB = getLocalDate();
const isDuplicate = all.some(r => 
r.userId === record.userId && 
(r.date || '').trim() === todayWIB // Pastikan compare dengan tanggal WIB hari ini
);
...
<div className="h-32 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-6 relative">
...
// FIX: Tambahkan Real-time Listener Global untuk Attendance
// Setup REAL-TIME LISTENER GLOBAL untuk attendance (sinkronisasi antar device)
const unsubscribeAtt = onSnapshot(getCollectionRef('attendance'), (snapshot) => {
const data = snapshot.docs.map(doc => doc.data());
localStorage.setItem(STORAGE.ATTENDANCE, JSON.stringify(data));
console.log("✓ Attendance synced real-time from Firestore");
}, (error) => {
console.error("Attendance sync error:", error);
// Fallback: sync manual jika listener gagal
